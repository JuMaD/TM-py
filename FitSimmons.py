...
for file in tqdm(os.listdir(dirname)):
    ...
    # get data from file
    voltage, currents = data_from_csv(filename, sep='\t', min_voltage=0.01, max_voltage=0.51, current_start_column=1,
                                      voltage_column=0)
    ...
    for i in range(1,len(currents)):
        ...
        result_to_csv(df, filename, f"simmons-fitresult_{i}.csv")
        result_to_csv(trials_df, filename, f"simmons-best_trials_{i}.csv")